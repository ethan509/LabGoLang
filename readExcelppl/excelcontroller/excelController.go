...
var FILE_NAME string = "TEST_1.xlsx"
...
var validatedNumberList ExcelInfoSlice
...
func SaveExcel() {
	err := resultFile.SaveAs(FILE_NAME)
	if err != nil {
		fmt.Println(err)
	}
}
...
func ReadExcel(orgFileName string) {
	...
	for rowNum, row := range readRows {
		...
		//WriteOneRowExcelSheet(SHEET_NAME, excelInfo, rowNum)
	}
...
func setNewPhoneNumber(excelInfo *common.ExcelInfo) bool {
	...
	if isChanged {
		...
		checkingNumber = excelInfo.NewPhoneNumber
		setDesc(excelInfo, "dash")
		//excelInfo.Desc = "dash"
		//delDashList = append(delDashList, *excelInfo)
		//WriteOneRowExcelSheet("org", excelInfo, excelInfo.ExcelIndex)
	}
	...
	if isChanged {
		excelInfo.NewPhoneNumber = newKey
		checkingNumber = excelInfo.NewPhoneNumber
		setDesc(excelInfo, "10")
		//excelInfo.Desc = "10"
		//start10List = append(start10List, info)
	...
	if isChanged {
		excelInfo.NewPhoneNumber = newKey
		checkingNumber = excelInfo.NewPhoneNumber
		setDesc(excelInfo, "70")
		fmt.Printf("70(%s)\n", excelInfo.NewPhoneNumber)
	}

	// step3. [delete] 최소길이 미만 skip
	if len(checkingNumber) < common.MIN_PHONE_NUMBER_LENGTH {
		setDesc(excelInfo, "min")
		//excelInfo.Desc = "min"
		//delete(common.ExcelInfos, excelInfo.PhoneNumber)
		//lessLenCnt++
	...
	if regExpNotStartX.Match([]byte(checkingNumber)) {
		//delete(common.ExcelInfos, info.PhoneNumber)
		//notMobileCnt++
		//fmt.Printf("notMobile(CheckingNumber:%s) -> (%s)\n" /*notMobileCnt,*/, checkingNumber, excelInfo.NewPhoneNumber)
		setDesc(excelInfo, "notMobile")
		//excelInfo.Desc = "notMobile"
		return false
	}
...
func WriteExcelSheet(sheetName string, list []common.ExcelInfo) {
	if resultFile.GetSheetIndex(sheetName) == 0 {
		resultFile.NewSheet(sheetName)
		resultFile.SetCellValue(sheetName, "A1", "기존 액셀 index")
		resultFile.SetCellValue(sheetName, "B1", "이름")
		resultFile.SetCellValue(sheetName, "C1", "휴대폰번호")
		resultFile.SetCellValue(sheetName, "D1", "지역")
...
func changeByRegexp(regExp *regexp.Regexp, checkingNumber string, chgStr string /*, cnt *int*/) (string, bool) {
	...
	if newPhoneNumber != checkingNumber {
		//keyChangeOfMap(info, changedKey /*, cnt*/)
		//fmt.Printf("[%v](%4d)(%s) -> (%s)\n", regExp, *cnt, oldPhoneNumber, changedKey)

		// if regExp.String() == "^16" {
		// 	fmt.Printf("[%v] -> (%s)\n", regExp, newPhoneNumber)
		// }
